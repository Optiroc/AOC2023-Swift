...
public struct Matrix<Element>: RandomAccessCollection {
    ...
    public var columns: some Sequence<Matrix<Element>> {
        Matrix.Columns(self)
    }

    public var rows: some Sequence<Matrix<Element>> {
        Matrix.Rows(self)
    }
...
public extension Matrix {
    func continuousMatricesSatisfying(_ predicate: (Element) -> Bool, maxLength: Int = .max) -> [Matrix] {
        self.continuousViewsSatisfying(predicate, maxLength: maxLength).map {
            self.slice($0)
        }
    }

    func continuousViewsSatisfying(_ predicate: (Element) -> Bool, maxLength: Int = .max) -> [MatrixView] {
        var views = [MatrixView]()
        for (rowIndex, row) in self.rows.enumerated() {
            var lastMatching = false
            var startIndex = 0
            for (columnIndex, element) in row.enumerated() {
                let currentMatching = predicate(element)
                if currentMatching, !lastMatching {
                    startIndex = columnIndex
                } else if !currentMatching, lastMatching {
                    views.append(MatrixView(
                        MatrixOffset(startIndex, rowIndex),
                        MatrixSize(columnIndex - startIndex, 1)
                    ))
                }
                if currentMatching, columnIndex == row.view.size.columns - 1 {
                    views.append(MatrixView(
                        MatrixOffset(startIndex, rowIndex),
                        MatrixSize(1 + columnIndex - startIndex, 1)
                    ))
                }
                lastMatching = currentMatching
            }
        }
        return views
    }
...
public extension Matrix {
    // TODO: RandomAccessCollection instead?
    struct Columns<U>: Sequence, IteratorProtocol {
        private let matrix: Matrix<U>
        private var index: Int = 0

        init(_ matrix: Matrix<U>) {
            self.matrix = matrix
        }

        public mutating func next() -> Matrix<U>? {
            if index >= matrix.view.size.columns { return nil }
            defer { index += 1 }
            return matrix.column(index)
        }
    }

    struct Rows<U>: Sequence, IteratorProtocol {
        private let matrix: Matrix<U>
        private var index: Int = 0
        ...
        public mutating func next() -> Matrix<U>? {
            if index >= matrix.view.size.rows { return nil }
            defer { index += 1 }
